...
- 💡 **Блок «Главное из видео» (Executive Summary / TL;DR)**:
  - Двухэтапная генерация: главы с таймкодами отображаются сразу, а во вложенной карточке сверху нейросеть YandexGPT выделяет верхнеуровневую суть видео.
  - Компактный дизайн с плавной кнопкой сворачивания/разворачивания.
  - Включается в буфер обмена при копировании текста пересказа.
- 💬 **Умный конвейер извлечения текста (YouTube Subtitles + Fallback)**:
  - Автоматически извлекает дорожки субтитров из плеера YouTube (`timedtext?fmt=json3`) с приоритизацией: русские ручные $\rightarrow$ русские авто (ASR) $\rightarrow$ английские $\rightarrow$ любые доступные.
  - Напрямую передаёт связный транскрипт в нейросеть YandexGPT (`type: "text"`), формируя **6–8 глубоких и содержательных тезисов** с фактами, именами и ключевыми выводами.
  - Автоматический Fallback: если у видео нет субтитров, скрипт суммаризирует главы и тезисы от распознавания речи Яндекса.
